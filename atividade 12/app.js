...
let cat = {
    ...
    age: '10',
    color: 'preto',
    bestfriends: ['samuel', 'pedro', 'arllen'],
    sound: 'miaaaaaaaaaaaauuuuuuuu'
}
...
/*
  ...
  - Exiba o array de amigos no console para verificar se o novo amigo(a) foi  
    adicionado.
*/
catbestfriends = ['samuel', 'pedro', 'arllen' ]
catbestfriends.push('eu')
console.log(catbestfriends)
...
/*
  ...
  - Exiba a nova cor do gato no console, também utilizando a sintaxe de  
    colchetes.
*/
cat['colors'] = [cat['color']];
cat['colors'].push('branco');
...
function getMessage(cat, dog) {
    let somaIdades = cat.age + dog.age;
    return `A soma das idades de ${cat.name} e ${dog.name} é ${somaIdades}.`;
}
...
*/
console.log("Nome do carro: " + car['name']);
console.log("Marca do carro: " + car['brand']);
console.log("Cores disponíveis: " + car['colors'].join(', '));
console.log(car.run()); 
console.log(car.stop()); 
console.log(car.getColorsMessage());
...
function classificarSituacao(risco) {
    if (risco === 1 || risco === 2) {
        return 'Risco leve';
    } else if (risco === 3 || risco === 4) {
...
for (let i = 1; i <= 20; i++) {
    let paciente = {
        ...
        cpf: `CPF ${i}`,
        sintoma: `Sintoma ${i}`,
        classificacaoRisco: Math.floor(Math.random() * 6),
    };
    paciente.situacaoRisco = classificarSituacao(paciente.classificacaoRisco);
